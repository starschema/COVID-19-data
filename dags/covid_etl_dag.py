...
def create_dag(dag_id, args):
    ...
    notebook_file = conf.get('core', 'dags_folder') + '/../notebooks/' + basename + ".ipynb"
    ...
    with dag:
        ...
        def execute_notebook():
            pm.execute_notebook(
                ...
                parameters=dict({"output_directory:": Variable.get("NOTEBOOK_OUTPUT_DIR") + "/"}),
                log_output=True,
                report_mode=True
            )
            return
...
i = 0
for file in os.listdir(conf.get('core', 'dags_folder') + '/../notebooks/'):
    if file.startswith("."):
        continue
